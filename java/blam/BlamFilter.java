...
class BlamFilter<T> {
  ...
  public BlamFilter(Funnel<T> funnel, long size) {
    this.funnel = funnel;
    this.size = size;
  ...
  private long hash(T object) {
    ...
    long unsigned = hash < 0 ? -hash : hash;
  ...
  private boolean isSizeCorrect() {
    return this.size == this.calculatedSize();
  }
  ...
  private boolean isEntrySet(long index) {
